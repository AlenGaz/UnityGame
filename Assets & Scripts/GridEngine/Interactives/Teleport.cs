...
public class Teleport : InteractiveObject
{
    ...
    public void startTeleportTrigger(GridObject gridObject, GridTile gridTile)
    {
        //OnEnterTileMethod(gridObject, gridTile);
        forceTeleport(gridObject);
    }


    public void forceTeleport(GridObject gridObject)
    {
        Debug.Log("In ForceTeleport");
        ...
        gridMovement.MoveTo(destinationTile, false, false);
    ...
    protected override void OnEnterTileMethod(GridObject gridObject, GridTile gridTile)
    {
        ...
        Debug.Log("Character entered Tile method");
       

        // Check if the target gridObject is teleportable
        if (IsTeleportable(gridObject))
        {
            Debug.Log("Character is teleportable");
            Activate(gridObject);
        }
        Activate(gridObject);
    }
