...
public class GridManager : MonoBehaviour
{
    ...
    [HideInInspector] public List<GridObject> m_GridObjects = new List ..........
    ...
    public Transform GridTilesHolder
    {
        get
        {
            if (_gridTilesHo..... null) { GetHolders(); }
    ...
    public virtual List<GridTile> WalkableNeighbors(GridTile gridTile, bool ignoresHeight = false, bool unoccupiedTilesOnly = true, GridTile goalTile = null, List<Vector2Int> customDirections = null)
    {
        ...
        foreach (Vector2Int dir in directions)
        {
            ...
            if (ExistsTileAtPosition(newVector))
            {
                ...
                GridObject targetGridTileGridObject = GridManager.Instance.GetGridObjectAtPosition(newVector); // added for mouseclick block
                if (targetTile != null)
                {
                    if (targetTile.m_IsTileWalkable || (goalTile != null && targetTile == goalTile))
                    {
                        if (unoccupiedTilesOnly && targetTile.IsTileOccupied() && (goalTile == null || (goalTile != null && targetTile != goalTile)))
                            continue;
                        // added for mouseclick block
                       /* if (targetGridTileGridObject != null)
                        {
                            if (targetGridTileGridObject.CompareTag("Spell")) {
                                Debug.Log("Stepping on spell tile");
                                targetTile.m_IsTileWalkable = true;
                            }
                            if (!targetGridTileGridObject.gameObject.CompareTag("Character") || !targetGridTileGridObject.gameObject.CompareTag("Enemy"))
                            {
                                continue;
                            }
                            else
    ...
    public virtual GridObject InstantiateGridObject(GridObject gridObjectPrefab, Vector2Int gridPosition, Orientations? initialOrientation = null, Transform targetParent = null, bool? checkTileAtPosition = true)
    {
        ...
        if (gridObjectAtPosition != null) // && gridObjectAtPosition.gameObject == gameObject
        {
        ...
        else
        {
            ...
            //instantiatedGridObject = gridObjectPrefab as GridObject;
#if UNITY_EDITOR
        }
#endif
    ...
    //instantiate ability 
    public virtual GridObject InstantiateGridAbility(GridObject gridObjectPrefab, Vector2Int gridPosition, Orientations? initialOrientation = null, Transform targetParent = null, bool? checkTileAtPosition = true)
    {
        if (gridObjectPrefab == null)
            return null;
        ...
        if (gridObjectAtPosition != null) // && gridObjectAtPosition.gameObject == gameObject
        {
            Debug.Log("There is another gridobject on where you are trying to instantiate ability."+ gridObjectAtPosition.gameObject.name);
    ...
    {
        return m_GridTiles.ElementAt(Random.Range(0, m_GridTiles.Count - 1)).Value;
    }
}