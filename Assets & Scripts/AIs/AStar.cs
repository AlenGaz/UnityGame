...
public class AStar //removed static from the class to be able to change movespeed inside here;
{

    public float normalMoveSpeed;
    public float diagonalMoveSpeed; // need to add here that diagonal map click movement is slower
    ...
    public static List<GridTile> Search(GridTile start, GridTile goal)
    {
        ...
        while (frontier.Count > 0)
        {
            ...
            foreach (GridTile next in GridManager.Instance.WalkableNeighbors(current, false, true, goal)) //current,false,false,goal to stop when smth in way
            {
                float new_cost = cost_so_far[current] + next.Cost();
                if (Utilities.Heuristic(current, next) > 1)
                {
                    new_cost += 10;
